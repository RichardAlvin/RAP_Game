...
class Player{
    ...
    draw(){
        ...
        c.fillStyle = this.freeCollision ? 'green' : 'yellow'
        c.fill()
        c.closePath()
        c.restore()
    }
...
function animate(){
    ...
    //detech collision between ghosts and player
    for(let i = ghosts.length - 1; 0 <= i; i--){
        ...
        if(Math.hypot(ghost.position.x - player.position.x, ghost.position.y - player.position.y) < ghost.radius + player.radius){
            if(ghost.scared){
                ghosts.splice(i, 1)
            }else if(live > 0 && !freeCollision){
                freeCollision = true
                player.freeCollision = freeCollision
                setTimeout(() =>{
                    live--;
            ...
            }else if(!freeCollision){
                alert('You Lose, please restart to play again!')
    ...
    if(pellets.length === 0){
        level++
        alert('You Win, go to level'+ level)
        cancelAnimationFrame(animationId)
        window.location.reload()
    }
...
alert('Game ini masih tahap pengembangan, gunakan W A S D di keyboard untuk bermain')
animate()
